...
import com.volodymyrchikh.abitandstudhelp.domain.QComment;
...
public class CommentService {
    ...
    public CommentResponse create(CommentRequest commentRequest) {
        User user = userRepository.findById(commentRequest.getUserId())
                .orElseThrow(() -> new UserNotFoundException("User with id=[%s] not found"
                        .formatted(commentRequest.getUserId()), commentRequest.getUserId()));

        Post post = postRepository.findById(commentRequest.getPostId())
                .orElseThrow(() -> new PostNotFoundException("Post of id=[%s] not found"
                        .formatted(commentRequest.getPostId()), commentRequest.getPostId()));
    ...
    public CommentResponse getById(Long commentId) {
        Comment comment = commentRepository.findById(commentId)
                .orElseThrow(() -> new CommentNotFoundException("Comment of id=[%s] not found".formatted(commentId), commentId));
    ...
    public CommentResponse update(Long id, UpdateCommentRequest commentRequest) {
        Comment comment = commentRepository.findById(id)
                .orElseThrow(() -> new CommentNotFoundException("Comment of id=[%s] not found".formatted(id), id));
